...
refFrame = max(imageArray(x,y,z), [], dim); 
...
alignedImArray=fourierCrossCorrelAlignment_mk2(imageArray,refFrame,filetype);
% getting an error? can you debug using a breakpoint inside the function?
...
figure;
plot(meanFaligned2); % do you get an error here? Can you fix it?
...
for iMask = 1:nMasks
    ...
    for iFrame = 1:nFrames
        currFrame = alignedImArray(:,:,x);
        maskSignals(y,z) = mean(currFrame(currMask));
    end
